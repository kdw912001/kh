...
-- 직급순 오름차순정렬하고, 직급이 같으면 이름순 오름차순정렬 처리함
...
컬럼명 NOT IN (다중행 서브쿼리) : 여러 개의 값중 한 개라도 일치하지 않는다면...

컬럼명 > ANY (다중행 서브쿼리) : 여러 값중 하나라도 크냐?
                    가장 작은 값보다 크냐?
컬럼명 < ANY (다중행 서브쿼리) : 여러 값중 하나라도 작은냐?
                    가장 큰 값보다 작으냐?
                    
컬럼명 > ALL (다중행 서브쿼리) : 모든 값보다 크냐? (가장 큰 값보다 크냐?)
컬럼명 < ALL (다중행 서브쿼리) : 모든 값보다 작으냐? (가장 작은값보다 작으냐?)
*/
