...
SELECT * FROM EMPLOYEE WHERE DEPT_ID = '50'; --MGR_ID로 관리자 확인
...
/*
메소드(리턴값이 있는 메소드())
컬럼명 비교연산자 비교할 값 <---비교할 값을 알아내기 위한 SELECT 문을
                            값 사용 위치에 바로 쓸 수 있음
컬럼명 비교연산자 (SELECT ....)
*/
...
--서브쿼리는 SELECT절, FROM 절, WHERE 절, GROUP BY절, HAVING절,
...
WHERE SALARY = (SELECT MIN(SALARY)
                FROM EMPLOYEE
                GROUP BY DEPT_ID);--ERROR--다중행 서브쿼리
...
컬렴명 IN (다중행 서브쿼리) : 여러 개의 값 중에서 한 개라도 일치하는 값이 있다면...
컬럼명 NOT IN (다중행 서브쿼리) : 여러 개의 값 중 한 개라도 일치하지 않는다면...
컬럼명 > ANY(다중행 서브쿼리) : 여러 값 중 하나라도 크냐?
                    가장 작은 값보다 크냐?
컬럼명 < ANY(다중행 서브쿼리) : 여러 값 중 하나라도 작으냐?
                    가장 큰 값보다 작으냐?
...
--IN / NOT IN
--여러 개의 값과 비교해서 같은 값이 있는지 / 같은 값이 없는지 비교
...
WHERE EMP_ID NOT IN (SELECT MGR_ID FROM EMPLOYEE
                    WHERE MGR_ID IS NOT NULL)--NOT IN은 NULL이 있으면 안됨
ORDER BY 3,1;
