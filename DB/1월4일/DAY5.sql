...
-- ORDER BY 절 ************************************
-- SELECT 한 컬럼을 기준으로 행들을 재배치 정렬시킬 때 사용함
-- ORDER BY 컬럼명 [ASC] | DESC    기본값이 오름차순
...
6 : ORDER BY 컬럼명 | 별칭 | 컬럼순번 정렬방식 [NULLS LAST | FIRST];
*/
...
-- 부서코드를 기준 내림차순 정렬하고, 같은 부서코드일 때는 입사일 기준으로 오름차순 정렬
-- 입사일도 같으면 이름 기준 오름차순 정렬 처리함

SELECT EMP_NAME 이름, HIRE_DATE 입사일, DEPT_ID 부서코드, SALARY 급여 
FROM EMPLOYEE
WHERE HIRE_DATE > TO_DATE('20030101','RRRRMMDD')--HIRE_DATE >'03/01/01';
...
SELECT JOB_ID, SUM(SALARY) 급여의합계, FLOOR(AVG(SALARY)) 급여의평균, CONUT(*) 직원수
FROM EMPLOYEE
GROUP BY JOB_ID;
...
--HAVING절*********************************
--GROUP BY 절 아래에 사용함
--반드시 GROUP BY 절과 함께 사용해야 함
--그룹 묶어서 계산한 그룹함수 결과값들에 대한 조건처리가 목적임

--HAVING 그룹함수(컬렴명) 비교연산자 비교값
--SELECT 절은 HAVING 처리로 골라낸 결과만 출력 처리함.

--부서별 급여합계 중 9백만을 초과하는 부서와 급여합게 조회
SELECT DEPT_ID, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_ID
...
--ROLLUP 함수
--GROUP BY 절에서만 사용함
--그룹별로 묶어서 계산한 결과에 대해서 총 집계를 구할 때 사용
SELECT DEPT_ID, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_ID;
...
SELECT DEPT_ID, SUM(SALARY)
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_ID); --제일 아래 행에 총 합을 추가 함.

SELECT DEPT_ID, SUM(SALARY), FLOOR(AVG(SALARY)), MIN(SALARY),
        MAX(SALARY)
FROM EMPLOYEE
WHERE DEPT_ID IS NOT NULL
GROUP BY ROLLUP(DEPT_ID);

--그룹함수로 계산된 결과값들에 대한 총집계가 자동으로 아래쪽에 행으로 추가됨.
...
FROM EMPLOYEE
WHERE DEPT_ID IS NOT NULL AND JOB_ID IS NOT NULL
GROUP BY CUBE(DEPT_ID, JOB_ID);--집계가 제일 위에 표시
...
--컬럼 그룹 묶을 때 사용함.
--그룹 묶음 값(0)인지, 집계값(1)인지 구분하기 위한 용도로 사용함
