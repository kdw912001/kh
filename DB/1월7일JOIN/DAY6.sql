...
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_ID = D.DEPT_ID;
--EMPLOYEE의 DEPT_ID와 DEPARTMENT의 DEPT_ID가 출력됨.
--그래서 DEPT_ID가 어디 테이블의 DEPT_ID인지 명시해야 함.

--ANSI 표준구문
--조인 처리를 위한 별도의 구문을 FROM절에 작성함
--모든 DBMS가 공통으로 사용하는 표준 구문임.
SELECT *
FROM EMPLOYEE
INNER JOIN DEPARTMENT USING (DEPT_ID); 
--DEPT_ID 컬럼이 맨 앞으로 옴, 
--연결에 사용할 컬럼이 하나로 합쳐지기 때문에 어디 테이블에 있는지 명시할 필요없음.
...
--컬럼명이 다르기 때문에 별칭으로 구분 지어줄 필요 없음
...
--EMPLOYEE 테이블의 모든 직원정보(NULL포함)를 JOIN에 모두 포함하고 싶으면 LEFT
LEFT JOIN DEPARTMENT USING (DEPT_ID);--OUTER 생략가능

--부서정보 모두 다 조인에 포함되게 하려면
SELECT *
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_ID(+) = D.DEPT_ID;
--DEPARTMENT에는 DEPT_ID =30이 있지만 EMPLOYEE 테이블에는 해당정보가 없기 때문에
--EMPLOYEE에 없는 행을 추가하기 위해 E.DEPT_ID(+) 해줌
...
--오라클 전용 구문에서는 FULL OUTER JOIN 사용할 수 없음
...
--NATURAL [INNER] JOIN
--연결할 테이블의 기본키(PRIMARY KEY)로 설정된 컬럼을 사용해서 조인 됨.
--오라클 전용구문에서는 없음 오직 ANSI
SELECT *
FROM EMPLOYEE
--NATURAL INNER JOIN DEPARTMENT;
NATURAL JOIN DEPARTMENT; --USING JOIN 한 것과 동일
--DEPARTMENT 의 기본키 컬럼이 DEPT_ID 를 사용해서 조인됨.

--NON EQUAL JOIN*************************
--저장하는 컬럼의 값이 일치하는 행들을 조인하는 경우가 아닌 조인.
...
--N개의 테이블 조인
--테이블 간의 관계(RELATIONSHIP)를 고려해서 순서를 정해야 함
...
--나이의 최소값 조회
SELECT MIN(TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,2),'RR')) / 12)) 나이
FROM EMPLOYEE;
...
SELECT EMP_ID 사번, EMP_NAME 사원명,
       MIN(TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,2),'RR')) / 12)) 나이,
       --(TO_NUMBER(TO_CHAR(SYSDATE,'RRRR'))- TO_NUMBER(LPAD(SUBSTR(EMP_NO,1,2),4,'19'))) 나이,
       DEPT_NAME, JOB_TITLE
FROM EMPLOYEE
LEFT JOIN DEPARTMENT USING (DEPT_ID)
LEFT JOIN JOB USING (JOB_ID)
GROUP BY EMP_ID, EMP_NAME, DEPT_NAME, JOB_TITLE
HAVING MIN(TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,2),'RR')) / 12)) = 30;
...
WHERE E.DEPT_ID = D.DEPT_ID
AND E.JOB_ID = J.JOB_ID
AND (SALARY+(SALARY*NVL(BONUS_PCT,0.0)))*12 > MIN_SAL;
...
JOIN JOB USING(JOB_ID)
JOIN DEPARTMENT USING(DEPT_ID)
WHERE (SALARY+(SALARY*NVL(BONUS_PCT,0.0)))*12 > MIN_SAL;
...
WHERE E.EMP_NAME <> M.EMP_NAME --사원명과 동료이름이 같을 때 제외
ORDER BY E.EMP_NAME;
--
--
