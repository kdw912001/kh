...
-- 구조
-- 값이 기록된 컬럼(키워드 : 검색에 사용되는 단어)과 값이 기록된 행위치(ROWID)로 구성됨
-- 키워드는 정렬이 된 상태로 구성됨
...
--UNIQUE INDEX 는 같은 값이 여러 번 기록된 컬럼을 키워드로 설정할 수 없음
...
--인덱스 관련 데이터 딕셔너리 : USER_INDEXEX, USER_IND_COLUMNS
DESC USER_INDEXES;
...
--인덱스의 사용
--SELECT 문 실행시 자동 사용됨.
--EMPL01과 EMPL02의 검색 속도 비교해 봄
SELECT * FROM EMPL01
WHERE EMP_ID = '141'; --0.003초

SELECT * FROM EMPL02
WHERE EMP_ID = '141'; --0.011초
