...
사용목적 : 
1. 보안에 유리 : 보관도니 결과 화면만 보여 줌으로써 쿼리문을 안 보이게 함.
2. 복잡하고 긴 쿼리문을 뷰를 통해 봄으로써 매번 쿼리문을 실행시키지 않아도 됨.
*/

-- 사용 형식
/*
CREATE [OR REPLACE] [FORCE | NOFORCE(기본값)] VIEW 뷰이름  
AS 서브쿼리
...
WHERE DEPT_ID = '90'; 
--권한이 없어서 실행이 안됨 SYS계정에서 권한부여 GRANT CREATE VIEW TO student;

--뷰 실행 확인
SELECT * FROM V_EMP_DEPT90; --VIEW 객체는 SELECT쿼리가 저장되어있으니 인라인뷰 개념임.

-- 딕셔너리 확인 : USER_VIEW, USER_OBJECTS, USER_CATALOG
DESC USER_VIEWS;

SELECT VIEW_NAME, TEXT_LENGTH, TEXT, READ_ONLY
FROM USER_VIEWS; --TEXT_LENGTH는 SELECT문 길이 --뷰 객체는 SELECT쿼리 저장
...
AS
SELECT EMP_NAME, DEPT_NAME, JOB_ID
FROM EMPLOYEE
LEFT JOIN JOB USING (JOB_ID)
LEFT JOIN DEPARTMENT USING (DEPT_ID)
WHERE JOB_TITLE = '사원';
...
CREATE OR REPLACE VIEW V_EMP_DEPT_JOB
AS
SELECT EMP_NAME ENAME, DEPT_NAME DNAME, JOB_ID JTITLE
FROM EMPLOYEE
LEFT JOIN JOB USING (JOB_ID)
LEFT JOIN DEPARTMENT USING (DEPT_ID)
WHERE JOB_TITLE = '사원';
...
FROM EMPLOYEE;
--함수식에 별칭 안 쓰면 ERROR >> 뷰 만들 때 별칭 혹은 서브쿼리 문에서 별칭 선언
...
VALUES ('789', '오현정', '811122-2234567', 'N');--EMPLOYEE 테이블에 INSERT가 됨
...
CREATE OR REPLACE NOFORECE VIEW V_EMP
AS
SELECT TCODE, TNAME, TCONTEST
FROM TTT; --ERROR
