...
* 쿼리 PARTITION 절 : PARTITION BY 표현식
            PARTITION BY 로 시작하며, 표현식에 따라서 그룹별로 분리하는 역할을 함.
...
--급여 2300000 이 급여 큰 값에서 작은 값 순으로 정렬되었을 때 순위를 조회하려면
...
--NTILE() ********************************
/*
PARTITION 을 버킷(BUCKET) 이라 불리는 그룹 별로 나누고,
...
-- ROW_NUMBER() *********************************
-- ROWNUM 과는 관계가 없음
-- 각 PARTITION 내의 값들을 ORDER BY 에 의해 정렬한 후에 그 순서대로 순번을 부여함
...
-- 윈도우의 시작 행에서 현재 위치 (CURRENT ROW) 까지의 합계를 구해서 "WIN2"에
       SUM(SALARY) OVER (PARTITION BY DEPT_ID
                         ORDER BY EMP_ID
