...
--컬럼명 > ALL (다중행 서브쿼리) : 컬럼값이 서브쿼리 결과값들과 비교해서 
--                              가장 큰 값보다 크냐?
--컬렴명 < ALL (다중행 서브쿼리) : 컬럼값이 서브쿼리 결과값들과 비교해서 
--                              가장 작은 값보다 작으냐?
...
SELECT JOB_ID, TRUNC(AVG(SALARY),-5) --10000단위까지 표시
FROM EMPLOYEE
GROUP BY JOB_ID;
...
--3. 상(호연)관 서브쿼리 사용 --실무에서 많이 사용, SQL문 길이 줄어듦
SELECT EMP_NAME, JOB_TITLE, SALARY
FROM EMPLOYEE E 
LEFT JOIN JOB J ON (E.JOB_ID = J.JOB_ID)
WHERE SALARY = (SELECT TRUNC(AVG(SALARY),-5)
...
--직원이 소속된 부서의 부서명이 큰 값부터 정렬되게 직원정보 조회
...
FROM (SELECT *
        FROM EMPLOYEE
        ORDER BY SALARY DESC) --정렬 후에 ROWNUM 이 부여
WHERE ROWNUM < 4;