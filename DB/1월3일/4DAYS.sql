...
-- 직원들의 이름, 입사일, 현재까지의 근무일 수, 근무개월수, 근무년수 조회
SELECT EMP_NAME 이름, HIRE_DATE 입사일,
        FLOOR(SYSDATE - HIRE_DATE) 근무일수, --실수데이터 정수로 변환
...
-- 데이터베이스 사용 언어가 한국어로 지정이 되어 있기 때문에
-- 요일이름에 한글 사용함
-- 요일이름에 영어 사용하면 에러남
SELECT NEXT_DAY(SYSDATE, 'FRIDAY')
FROM DUAL; --ERROR

--요일 이름에 영어 사용하려면 언어를 변경해야 함
ALTER SESSION
...
ALTER SESSION
SET NLS_LANGUAGE = KOREAN; --다시 원상복구

COMMIT;

--LAST_DAY('날짜데이터' | 날짜가 기록된 컬럼명)
--지정한 날짜의 월에 대한 마지막 날짜 리턴
...
SELECT SYSDATE, 
        SYSTIMESTAMP,--년월일시분초
        CURRENT_DATE,--SYSDATE와 동일
        CURRENT_TIMESTAMP--년월일시분초
FROM DUAL;
...
SELECT EMP_NAME, HIRE_DATE, 
        ...
        FLOOR(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/12) 근무년수 
        --개월수로 계산하여 정확하기 떄문에 MONTH_BETWEENT을 많이 사용
FROM EMPLOYEE;
...
--자동형변환이 안 되는 경우
SELECT SYSDATE - '15/03/25' --'15/03/25'가 날짜로 자동형변환이 안됨.
FROM DUAL; --ERROR
--직접 형변환해야 함
...
SELECT TO_CHAR(SYSDATE , 'YEAR,Q') FROM DUAL; --Q는 분기
...
--날짜 데이터 비교연산시 시간을 가진 값데 대해 날짜만 비교 연산할 수 없음.
SELECT EMP_NAME, HIRE_DATE
FROM EMPLOYEE
