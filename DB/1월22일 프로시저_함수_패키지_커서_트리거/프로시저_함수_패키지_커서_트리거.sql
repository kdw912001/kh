...
-- IN 모드, OUT 모드 배개변수가 있는 프로시저 만들기
...
--부서번호를 전달받아, 해당 부서를 삭제하는 프로시저 작성
...
BEGIN
    DELETE 
    FROM DEPT_COPY
    WHERE DEPT_ID LIKE VDID;
    COMMIT;
END;
/

EXECUTE DEL_DEPTID('&부서코드');
...
IS
BEGIN
    DELETE FROM EMP_COPY WHERE EMP_NAME = VENAME;
    COMMIT;
    DBMS_OUTPUT.PUT_LINE(VENAME || ' 직원이 퇴사하였습니다.');
END;
/
SET SERVEROUTPUT ON;
EXECUTE DEL_ENAME('&직원이름');
SELECT * FROM EMP_COPY;
...
-- ************************
-- 커서 (CURSOR)
-- SELECT 쿼리문의 실행 결과 (RESULT SET)에 대한 결과행의 갯수가 여러 개 일 때
-- 결과행을 하나 씩 다루고자 할 때 사용하는 객체임
-- 자바의 ArrayList의 Index(순번) 와 비슷한 개념임.
...
--실습 : EMPCPY 테이블에 INSERT가 수행이 되고 나면, 자동으로 실행되는
--트리거 작성 : TR_WELCOME
...
-- 상품 테이블의 재고수량이 25에서 20으로 변경되게 해야 함
