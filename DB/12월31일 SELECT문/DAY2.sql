...
오라클 자료형
 ...
 - NUMBER : 기본은 정수형임, 기본자릿수도 7임.
    ...
    NUMBER(최대기록자릿수, 소숫참아래 자릿수) : 실수형 값을 기록
     => NUMBER(10, 2) : 실수형 값을 최대 10자리까지 기록
                        소수점 아래 자릿수는 2째자리까지 기록가능
...
-- 사용 2
-- 직원 테이블에서 관리자 사번이 100인(팀장이 한선기 인) 직원 조회
SELECT * FROM EMPLOYEE 
WHERE MGR_ID='100'; --SQL에서는 ==이 아니라 =사용
...
-- 사용 4
-- 직원 테이블과 부서 테이블(DEPARTMENT)에서 정호 보회
-- 사번, 이름, 급여, 부서명 조회
...
/*
기본구문
SELECT * | [DISTINCT] 조회할 컬럼명     DISTINCT는 중복을 제거
FROM 조회에 사용할 테이블명;
*/
...
FROM EMPLOYEE;
--DISTINCT는 SELECT 절에 딱 1번만 사용할 수 있음
...
--직원 테이블에서 사번, 사원명, 급여, 보너스포인트가 적용된 연봉 조회
--계산식 : (급여+(급여*보너스포인트))*12
SELECT EMP_ID, EMP_NAME, SALARY, (SALARY + (SALARY*BONUS_PCT)) *12
FROM EMPLOYEE;
-- 계산값에 NULL 이 있으면 결과도 NULL 이 됨.

-- 계산식에 함수 사용할 수 있음
-- 컬럼의 값이 NULL 인 경우 다른 값으로 바꾸는 함수 : NVL 
-- NVL(컬럼명, 바꿀값)
SELECT EMP_ID, EMP_NAME, SALARY, 
...
SELECT EMP_ID 사번, EMP_NAME 이름, SALARY AS "급여(원)",--별칭에 공백, 숫자, 기호문자가 있을 때 반드시 " "
...
-- 급여가 2백만 이상 4백만 이하인 직원의 정보 조회
-- 사번, 이름, 급여, 직급코드, 부서코드 조회, 별칭
...
-- 날짜 데이터 표기시에는 데이터베이스가 정한 날씨 포맷에 맞춰서 
-- '날짜형식' 묶어서 표기해야 함
...
-- 90번 부서에 소속된 직원 중에서 급여를 2백만보다 많이 받는 지원 조회
SELECT *
FROM EMPLOYEE
...
-- 연결 연산자 : || (자바의 문자열 합치기하는 + 와 같은 의미의 연산자임)
-- SELECT 절에서 조회한 컬럼 값들을 하나로 합치거나, 컬럼값과 리터럴을 합칠 경우
...
-- 급여값 뒤에는 '(원)' 리터럴 붙여줌
-- 결혼여부는 리터럴을 채움 : '기혼' 이 출력되게 적용함
SELECT EMP_NAME 직원의이름, HIRE_DATE 입사일, JOB_ID 직급코드, 
        DEPT_ID 부서코드, SALARY || ' (원)' 급여, '기혼' 결혼여부 
FROM EMPLOYEE
WHERE HIRE_DATE >= '00/01/01'  AND MARRIAGE = 'Y'; --한선기는 입사일이 2090/04/01이기에 나옴

--급여를 2백만보다 적게받거나, 4백만보다 많이 받는 지어ㅜㄴ 조회
--이름, 급여 조회
...
-- LIKE 연산자
-- 문자열 값에 패턴을 제시해서, 패턴과 일치하는 값을 골라낼  사용하는 연산자임
...
--부서 배치는 받지 않았는데, 보너스 포인트는 받는 직원 조회
SELECT *
FROM EMPLOYEE
...
FROM EMPLOYEE
--WHERE DEPT_ID = '20' OR DEPT_ID = '90' AND SALARY >3000000; 연산자 우선순위 문에 AND가 먼저 수행됨
--WHERE (DEPT_ID = '20' OR DEPT_ID = '90') AND SALARY >3000000;
WHERE DEPT_ID IN('20', '90') AND SALARY >3000000; --IN 연산자가 AND보다 우선순위가 높음. 
--연산자 우선순위에 맞게 순서 정렬
