...
public class MethodSample {
	//Field
	//접근제한자 [예약어] 자료형 변수명; 예약어:final, static 
	...
	//예약어 : abstract, final, synchronized -> 요약파일에서 확인
	
	//1. 반환값 없고 매개변수도 없는 메소드
	public void printLotto() { //메소드 머리 혹은 메소드
		int[] lotto = new int[6];
		
		for(int i=0; i<lotto.length;i++){
			lotto[i] = (int)(Math.random()*45)+1;
			if(i>0) {	//중복값 체크 0일땐 할 필요없음
				for(int j=0; j<i; j++) {
					if(lotto[j]==lotto[i]) {
						i--;
						break;
