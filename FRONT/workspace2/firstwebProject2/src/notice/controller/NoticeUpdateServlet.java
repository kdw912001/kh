...
public class NoticeUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 3. 업로드할 파일의 저장 폴더 지정
		// 저장 폴더가 현재 구동중인 웹컨테이너(WAS(Web Application Server): 톰캣)에서
		// 실행중인 웹애플리케이션 안에 있는 폴더일 경우
		String root = request.getSession().getServletContext().getRealPath("/");// 구동중인 프로젝트의 root까지의 경로
		...
		// 서비스 모델로 notice 객체 전달하고 결과 받기
		int result = new NoticeService().updateNotice(notice);

		// 받은 결과를 가지고 성공/실패 뷰 선택 내보내기
		if (result > 0) {
			// 공지글 수정 성공시에는 공지글 상세보기 출력
			response.sendRedirect("/first/ndetail?no=" + notice.getNoticeNo());//sendRedirect는 html에서 href와 동일
