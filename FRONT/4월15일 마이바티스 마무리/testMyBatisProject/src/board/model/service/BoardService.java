...
public class BoardService {
	...
	public void addReadCount(int bnum) {
		SqlSession session = getSession();
		int result = bdao.addReadCount(session, bnum);
		if(result > 0)
			session.commit();
		else
			session.rollback();
	...
	public Board selectBoard(int bnum) {
		SqlSession session = getSession();
		Board board = bdao.selectBoard(session, bnum);
		session.close();
		return board;
	}
	...
	public int insertReply1(Board replyBoard) {
		SqlSession session = getSession();
		int result = bdao.insertReply1(session, replyBoard);
		if(result > 0)
			session.commit();
		else
			session.rollback();
	...
	public int insertReply2(Board replyBoard) {
		SqlSession session = getSession();
		int result = bdao.insertReply2(session, replyBoard);
		if(result > 0)
			session.commit();
		else
			session.rollback();
	...
	public int deleteBoard(int bnum) {
		SqlSession session = getSession();
		int result = bdao.deleteBoard(session, bnum);
		if(result > 0)
			session.commit();
		else
			session.rollback();
